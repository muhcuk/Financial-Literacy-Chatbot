...
def clean_text(raw: str) -> str:
    ...
    for ln in lines:
        ...
        if any(re.match(p, stripped_ln, flags=re.IGNORECASE) for p in HEADER_FOOTER_PATTERNS):
            continue
        if _looks_like_noise(stripped_ln):
            continue
...
def main():
    ...
    OUTPUT_DIR = "data/clean_chunks"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    files = glob(os.path.join(INPUT_DIR, "*.jsonl"))
    if not files:
        print(f"No JSONL files found in {INPUT_DIR}. Run pdf_to_rag.py first.")
        return
